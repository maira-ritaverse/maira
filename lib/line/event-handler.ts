...
async function handleFollow(
  ...
  // ===== display_name の 自動 リンク は 廃止 (成りすまし 対策) =====
  //
  // 従来 は 「LINE display_name が CRM 顧客名 に 完全 一致 する 場合、 1 件 だけ なら
  // 自動 で 紐付ける」 実装 だった が、 攻撃者 が LINE 表示 名 を 実在 顧客 名 に 変えて
  // 友達 追加 する だけ で 履歴書 案 / 求人 情報 の 全 outbound を 傍受 できる 経路 と
  // 判明 (監査 H2)。 監査 ログ 上 も 正常 な マッチ として 記録 され 事後 検知 が 困難。
  //
  // 代替: link_code / LIFF login / 「CRM に 追加」 ボタン の 手動 導線 のみ に 集約。
  // display_name の 完全 一致 検知 UI は 追って 「候補 提示 + admin 明示 承認」 で 復活
  // 予定。 それ まで は 手動 リンク の 摩擦 を 受け 入れる。
