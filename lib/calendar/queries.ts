...
export async function listCalendarEvents(
  ...
  // 並列で 4 ソース + クライアント名マップを取得
  const [clientsRes, tasksRes, interactionsRes, meetingsRes] = await Promise.all([
    supabase
      .from("client_records")
  ...
  // タスク / 対応履歴側に未収録のクライアントが居れば追加で取得する。
  ...
  const missingIds = Array.from(new Set([...taskClientIds, ...interactionClientIds])).filter(
    (id) => !clientNameMap.has(id),
  );
