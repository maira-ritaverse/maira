...
const PRESERVE_PROMPT = [
  ...
  "- Eyes: keep the exact eye shape, eyelid crease (single/double), iris color, pupil size, eyebrow shape and density, eyelashes. Do NOT preserve the input's gaze direction or head angle — the gaze is re-oriented straight to the camera per HEAD ORIENTATION below, which is NOT an identity change.",
  ...
  "- HEAD ORIENTATION IS MANDATORY: the face and head must point STRAIGHT at the camera (frontal ID-photo pose). If the input shows the head tilted, turned, angled, looking away, or in profile, RE-ORIENT it to face the camera directly — nose pointing at the lens, both eyes looking into the lens, shoulders squared to the camera. This re-orientation is REQUIRED for a valid Japanese ID photo and does NOT count as an identity change: keep every facial feature identical and only correct the orientation.",
  "- Subject centered and upright, no head tilt, no roll, no yaw, no profile or three-quarter angle.",
  ...
  "- The head is tilted, turned to the side, in a three-quarter or profile view, or the gaze is not directed straight at the camera.",
