...
export async function sendLeadRequestNotificationToOperator(
  ...
  const subject = `【Myaira 資料 請求】${payload.companyName} / ${payload.contactName} 様`;

  const text = [
    `Myaira の 資料 請求 が 届きました。`,
    ``,
  ...
  const body = `
<h2 style="margin:0 0 12px;font-size:18px;line-height:1.4;">Myaira 資料 請求</h2>
${infoCard(
...
export async function sendLeadRequestAutoReply(
  ...
  const subject = `【Myaira】 資料 請求 を 受け付け ました`;

  const text = [
    `${payload.contactName} 様`,
    ``,
    `この 度 は Myaira の 資料 を ご請求 いただき ありがとう ござい ます。`,
    ``,
...
<p style="margin:0 0 12px;color:#333;line-height:1.7;font-size:14px;">
  ...
  この 度 は Myaira の 資料 を ご請求 いただき ありがとう ござい ます。<br>
  1 営業 日 以内 に 担当 から 資料 PDF と 簡単 な ご紹介 を お送り します。
</p>
...
<div style="margin:20px 0 8px;text-align:center;">
  ${primaryButton("https://www.maira.pro", "Myaira を 詳しく 見る")}
</div>
<p style="margin:24px 0 0;font-size:12px;color:#888;">Myaira 運営 チーム</p>
`.trim();
