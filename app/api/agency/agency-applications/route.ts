...
import { createAgencyApplicationRequestSchema } from "@/lib/agency-client-documents/types";
...
export async function GET(request: Request) {
  ...
  const items = await listAgencyApplications(clientRecordId, organization.id);
...
export async function POST(request: Request) {
  ...
  const parsed = createAgencyApplicationRequestSchema.safeParse(body.body);
  if (!parsed.success) {
  ...
  const result = await createAgencyApplication({
    ...
    referralId: parsed.data.referral_id,
    details: parsed.data.details,
    status: parsed.data.status,
    appliedAt: parsed.data.applied_at,
    appliedByMemberId: member.id,
  });
  if ("error" in result) {
