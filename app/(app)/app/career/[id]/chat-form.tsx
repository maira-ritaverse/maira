 ...
 * - 通信は DefaultChatTransport で /api/career/chat に向け、conversationId を body に同梱
 * - AI SDK v6:useChat に api/body/initialMessages を直接渡すAPIは廃止されたため、
 *   transport を明示する形に変えている
 */
export function CareerChatForm({ conversationId, initialMessages }: Props) {
  const [input, setInput] = useState("");
  const scrollRef = useRef<HTMLDivElement>(null);
  const openerSentRef = useRef(false);

  // 初期メッセージを useChat 用の UIMessage 形式に変換
  // initialMessages は props として固定なので useMemo で1回だけ計算
  ...
  // transport も useMemo で固定。conversationId が変わったら再生成する
