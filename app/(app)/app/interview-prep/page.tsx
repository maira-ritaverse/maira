 ...
 * /app/interview-prep
 *
 * 求職者本人向け:エージェントが自分のために作成し「共有」した面談対策の一覧。
...
export default async function SeekerInterviewPrepListPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">面談対策</h1>
        <p className="text-muted-foreground mt-1 text-sm">
          担当エージェントがあなたの選考のために作成した面談対策です。共有されたもののみ表示されます。
        </p>
      </div>
      ...
      {preps.length === 0 ? (
        <Card className="p-6">
          <p className="text-muted-foreground text-sm">
            まだ共有された面談対策はありません。担当エージェントが作成・共有すると、ここに表示されます。
          </p>
        </Card>
      ) : (
            ...
            <Link key={p.referralId} href={`/app/interview-prep/${p.referralId}`} className="block">
              <Card className="hover:bg-accent/40 p-4 transition-colors">
                <div className="text-muted-foreground mb-1 text-xs">
                  ...
                  {p.sharedAt && <> ・ {new Date(p.sharedAt).toLocaleDateString("ja-JP")} 共有</>}
                </div>
