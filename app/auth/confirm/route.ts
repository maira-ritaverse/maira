 ...
 *
 * ★Safari ダウンロード事故対策(重要):
 *   ルートハンドラが未捕捉例外を投げると Next.js は「Content-Type ヘッダの無い」
 *   500 応答を返す。この応答には X-Content-Type-Options: nosniff が付くため、
 *   Safari は型不明の本文を application/octet-stream 扱いにして「ファイルとして
 *   ダウンロード」してしまい、ユーザーは /reset-password に到達できず認証が
 *   完了しない(実例: 環境変数 PW_RESET_TICKET_SECRET 未設定で issuePwResetTicket
 *   が throw するケース)。そのため本ハンドラ全体を try/catch で保護し、想定外の
 *   例外は 500 ではなく通常のリダイレクトに倒す。
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    // 想定外の例外(例: issuePwResetTicket の PW_RESET_TICKET_SECRET 未設定)を
    // ここで捕捉する。未捕捉のままだと Next.js が Content-Type 無しの 500 を返し、
    // nosniff と相まって Safari が応答をファイルとしてダウンロードしてしまう。
    // 500 ではなく通常リダイレクトに倒すことで、その事故を根本的に防ぐ。
    console.error("[auth/confirm] unexpected error; redirecting to login", {
      type,
      message: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.redirect(failUrl);
