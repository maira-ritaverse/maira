...
2. 音声面接シミュレーター(β版でテキストモック実装済、本格ローンチで音声 I/O 拡張)
3. 会議録音 → AI による履歴書 / 職務経歴書の自動生成
   (Zoom / Google Meet 連携 + 音声ファイルアップロード)

**暗号化の方針(2026-06-15 改定)**:

- 当初目指していた「クライアントサイド暗号化(運営者も復号できない)」は
  会議録音・AI 処理機能と両立できないため断念。
- 機密性の高いフィールド(会話履歴 / キャリア棚卸し / 推薦コメント 等)は
  サーバーサイド AES-256-GCM 暗号化(現行)で保護。
- 運営者によるアクセスは「保管」「本人同意済の AI 処理」「法令対応」の
  範囲に限定する旨をプライバシーポリシーに明記し、登録時に同意を取る。
- 詳細は docs/adr/0006-server-side-encryption-direction.md 参照。
...
- サーバーサイド対称暗号: AES-256-GCM(Web Crypto API、Edge / Node 両対応)
- 鍵管理: 環境変数 `FIELD_ENCRYPTION_KEYS`(version map)+ `FIELD_ENCRYPTION_CURRENT_VERSION`
- IV: レコード単位でランダム生成
- 保存形式: `v{n}:base64url(iv ‖ ct+authTag)`
- 実装: lib/crypto/field-encryption.ts

### 絶対にやってはいけないこと

- 平文をDB(Supabase)に保存しない(暗号化対象列は必ず暗号化してから保存)
- console.logで復号後の機密データを出力しない
- 暗号化を「後で実装します」とコメントしてスキップしない
- 鍵を localStorage / sessionStorage / コードに含めない(サーバ側 env のみ)

### AI推論時の扱い

- サーバ側で復号 → Anthropic API へ送信(ブラウザに平文を渡さない)
- 推論結果は再度暗号化してから DB に保存
