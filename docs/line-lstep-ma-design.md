...
Maira の既存 LINE 連携 + MA 基盤の上に、「求職者への効果的なマーケティング」を実現する Lステップ相当の機能群(多段ステップ配信・条件分岐・動的セグメント・CV 追跡・行動スコア・キーワード自動応答・流入計測・フォーム・リッチメニュー自動化)を積み増す。

---

## 1. 前提

- 実装先:Maira 本体(別プロジェクトではない)
