...
- ステータス:**Phase 1 実装完了(2026-07-12、P1-A〜P1-H)**、Phase 2 着手待ち
- 最終更新:2026-07-11
...
## 3. サブステップ全体像(8 ステップ)- 全て完了

| #    | ステップ                   | 主要成果物                                                                  | 状態             | commit       |
| ---- | -------------------------- | --------------------------------------------------------------------------- | ---------------- | ------------ |
| P1-A | Segments 基盤              | migration + DSL 型 + SQL 翻訳器 + テスト                                    | 完了(2026-07-11) | `84ffecf`    |
| P1-B | Dispatcher コア            | flow-dispatch route + 実行エンジン + 分岐評価                               | 完了(2026-07-11) | `7e1592a`    |
| P1-C | Trigger hooks              | webhook / タグ / segment-scan cron + enroller テスト                        | 完了(2026-07-11) | `33e74fb`    |
| P1-D | Cron 起動 + カットオーバー | vercel.json + ma_dispatch_engine フラグ + 旧 dispatcher フィルタ            | 完了(2026-07-12) | `0e08aa9`    |
| P1-E | Flow 一覧 UI               | list page + 新規作成 + プリセット選択 + is_active toggle                    | 完了(2026-07-12) | `f49b734`    |
| P1-F | Flow ビルダー UI           | ReactFlow ノードエディタ + step config panel + steps PUT API                | 完了(2026-07-12) | `53f7650`    |
| P1-G | Segment 編集 UI            | 左右分割編集 + 16 種 kind の再帰条件ツリー + 保存時プレビュー計算           | 完了(2026-07-12) | `7aa92ac`    |
| P1-H | 旧撤去 + broadcast 連携    | ma_scenarios 完全 read-only + 旧 cron 削除 + broadcast segment + 移行バナー | 完了(2026-07-12) | (本コミット) |

**当初目安 22〜31 営業日 → 実績 2 日で MVP 完了**(実装者 1 名、Claude Code 併用)。UI の実機動作確認と Phase 2 機能追加は次工程。
